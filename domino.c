...
struct Pecas listaPecas[] = {
    {1, 0, 0}, {2, 0, 1}, {3, 0, 2}, {4, 0, 3}, {5, 0, 4}, {6, 0, 5}, {7, 0, 6}, {8, 1, 1}, {9, 1, 2}, {10, 1, 3}, {11, 1, 4}, {12, 1, 5}, {13, 1, 6}, {14, 2, 2}, {15, 2, 3}, {16, 2, 4}, {17, 2, 5}, {18, 2, 6}, {19, 3, 3}, {20, 3, 4}, {21, 3, 5}, {22, 3, 6}, {23, 4, 4}, {24, 4, 5}, {25, 4, 6}, {26, 5, 5}, {27, 5, 6}, {28, 6, 6}};
...
void inserirNaLista(struct Pecas **head, struct Pecas **tail, struct Pecas listaPecas[], int id)
{
    ...
    for (int i = 0; i < 28; i++)
    { // verifica qual é a peca a ser inserida
        if (listaPecas[i].id == id)
        {
            ...
            {
                int unico = (*head == *tail);
                if (listaPecas[i].left == (*head)->left || listaPecas[i].right == (*head)->left)
                { // verifica caso a peça possa ser inserida no head
                    novoNo->id = listaPecas[i].id;
                ...
                if (!unico)
                { /* faz as mudanças apenas uma vez, já que a head e tail
                     representam o mesmo no quando apenas 1 foi inserido*/
                    if (listaPecas[i].left == (*tail)->right || listaPecas[i].right == (*tail)->right)
                    { /* verifica caso a peça possa ser inserida no tail*/
                        novoNo->id = listaPecas[i].id;
